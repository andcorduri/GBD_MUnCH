...
#This version is for very long files that cannot be loeaded all at once. 
#So it loads chunks of the file at different levels of resolution.
#This version also uses the CPU Numba jit compiler. Is much faster that the uncompiled version.
...
#Read data
def readdata(stream, datat, datax, limi, lims, sens, m, i):
    k=0
    for j in range(limi,lims):
        line=stream.readline()
        k+=1
        if k==m**i:
            datat[int(j/m**i)]=float(line.strip().split(',')[0])
            datax[int(j/m**i)]=float(line.strip().split(',')[1])*sens
            k=0 
            
#refill the vector that contains measurement errors          
@jit
def drbVfun(drbV, datax, lim, sens, dsens):
...
#argument "datafile" is the name of the data file, The second argument "sens" is the
...
def msdsamp(datafile, datalength, loadlength, sens, dsens, dV):
    dataStr=open(datafile,'r')
    p=8
    m=2
...
msdt=msdsamp('./Vdata_traj_1.csv',2*10**6,1*10**4,sens,dsens,dV)
...
fig.savefig('msd_Gs_py.eps', format='eps', transparent =False, dpi=300, bbox_inches='tight',pad_inches=0.5)
