...
input4=[]
with open('./kernelp.csv','r') as csvfile:
    in4=csv.reader(csvfile, delimiter=',')
    for row in in4:
        input4.append(row)  
...
#Parameters for the inertialess memory kernel, used as input in the BD simulation       
LV=[]
for i in range(0,len(input4[0])):
       LV.append(float(input4[0][i]))
cV=[]
for i in range(0,len(input4[1])):
       cV.append(float(input4[1][i]))
...
#matrix B in Ornstein-Ulembech process
def matbf(c, l, u):
    mat=[]
    for i in range(0,len(c)):
        row=[]
        for j in range(0,len(c)):
            if i==j:
                row.append(math.sqrt(2*u**2/(c[j]*l[j])))
            else:
                row.append(0.0)
        row.append(0.0)
        mat.append(row)
    row=[]
    for i in range(0,len(c)):
        row.append(math.sqrt(2*c[i]*l[i]))
    row.append(math.sqrt(2*(u-sum(list(map(lambda x, y: x * y, c, l))))))
    mat.append(row)
    return mat
...
def initcond(c,l,u):
    vec=[]
    for i in range(0,len(c)):
        vec.append(np.random.normal(0,math.sqrt(u**2/c[i])))
    vec.append(np.random.normal(0,1.0))
    return vec


#Euler time steping for the Ornstein-Ulembech process
def timestep(vec, mata, matb, deltat, lens, sens, Tin, traps, datat, datax, limi, lims, m, i):
    k=0
    for j in range(limi,lims):
        wiener = np.random.normal(0,math.sqrt(deltat),len(matb[0]))
        brownian = np.dot(matb,wiener)
        spring = np.dot(mata,list(map(lambda x: x * deltat, vec)))
        vec = list(map(lambda x, y, z: x - y + z, vec, spring, brownian))
        k+=1
        if k==m**i:
            datat[int(j/m**i)]= deltat*j/m**i
            datax[int(j/m**i)]= vec[-1]*lens/sens*sens
            k=0
    return vec
            
#refill the vector that contains measurement errors          
@jit
def drbVfun(drbV, datax, lim, sens, dsens, dV):
    for i in range(0,lim):
        drbV[i]=math.sqrt((datax[i]*dsens)**2+(sens*dV)**2)
...
#the argument datalenth is the total number of data points in the data file
#the argument loadlength is the number of data points to load

def msdsamp(c, l, u, deltat, datalength, loadlength, sens, dsens, dV, Tin, traps):
    p=8
    m=2
    ...
    T=Tin+273.15
    He=traps*10**(-12)/10**(-9)
    lens=math.sqrt(kB*T/He)*10**9
    vec=timestep(vec,mata,matb,deltat,lens,sens,Tin,traps,datat,datax,0,p*g*m,m,0)
    drbVfun(drbV, datax, p*m*g, sens, dsens, dV)
    sampf=1/deltat
    ...
    if uplim < 2:
        ...
        for i in range(1,uplim):
            samp(datat, p, g, m)
            samp(datax, p, g, m)
            vec=timestep(vec,mata,matb,deltat,lens,sens,Tin,traps,datat,datax,p*g*m**i,p*g*m**(i+1),m,i)
            drbVfun(drbV, datax, p*m*g, sens, dsens, dV)
            for j in range(p*m**i,p*m**(i+1),m**i):
...
msdt=msdsamp(cV, LV, 1/z0in, 0.001, 2*10**6,1*10**4,sens,dsens,dV,TExp,HeExp)
...
plt.ylim(0.6*msdtt[0],2.0*msdtt[len(msdtt)-1])
...
plt.fill_between(wGs, GpUp, GpLo,alpha=0.001, edgecolor='salmon', facecolor='salmon',label=r'$G^\prime$')
plt.fill_between(wGs, GppUp, GppLo,alpha=0.001, edgecolor='violet', facecolor='violet',label=r'$G^{\prime\prime}$')
...
fig.savefig('msd_Gs_py.eps', format='eps', transparent =False, bbox_inches='tight',pad_inches=0.5)
